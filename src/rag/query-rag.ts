...
export class QueryRAG {
    public embedder: E5Embedder;
    public vectorStore: VectorStore;
    public queryData: QueryData;
    public isInitialized: boolean;
    public silent: boolean;

    constructor(options: QueryRAGOptions = {}) {
   ...
   * 모든 쿼리를 벡터 저장소에 인덱싱
   */
  async indexQueries(): Promise<void> {
    try {
   ...
   */
  async searchSimilarQueries(naturalLanguageQuery: string, topK: number = 5): Promise<Array<{id: string; similarity: number; description: string; sqlScript: string; metadata: any;}>> {
    if (!this.isInitialized) {
   ...
   */
  async updateQuery(queryId: string, updates: Partial<{ description?: string; sqlScript?: string; metadata?: any }>): Promise<void> {
    try {
