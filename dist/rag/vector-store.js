export class VectorStore {
     ...
     */
    async addVectors(vectors, queryIds) {
        if (!this.isInitialized) {
    ...
    normalizeVector(vector) {
        ...
        if (norm === 0)
            return vector.slice(); // 영벡터는 그대로 반환
     ...
     */
    fastCosineSimilarity(normalizedA, normalizedB) {
        let dotProduct = 0;
     ...
     */
    async updateVector(queryId, newVector) {
        const index = this.metadata.findIndex(meta => meta.id === queryId);
        if (index === -1) {
            throw new Error(`Query ID not found: ${queryId}`);
        }
